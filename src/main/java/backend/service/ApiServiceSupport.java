...
import static backend.context.Context.tokenSupport;
...
public class ApiServiceSupport {
    ...
    @Step("POST {endpoint}")
    public static <T> T postToPojoJsonPathData(String endpoint, Object body, Class<T> pojoClass) {
        return given()
                .auth().oauth2(tokenSupport)
                .body(body)
                .post(endpoint)
    ...
    @Step("GET {endpoint}")
    public static <T> T getToPojoJsonPathDataFirstIndexOfList(String endpoint, Class<T> pojoClass, Map<String, Object> params) {
        return given()
                .auth().oauth2(tokenSupport)
                .queryParams(params)
                .get(endpoint)
                .then()
                .extract().body().jsonPath().getObject("data[0]", pojoClass);
    }

    @Step("GET {endpoint}")
    public static <T> T getToPojoJsonPathDataFirstIndexOfList(String endpoint, Class<T> pojoClass) {
        return given()
                .auth().oauth2(tokenSupport)
                .get(endpoint)
                .then()
    ...
    @Step("POST {endpoint}")
    public static <T> List<T> postToListPojoJsonPathData(String endpoint, Object body, Class<T> pojoClass) {
        return given()
                .auth().oauth2(tokenSupport)
                .body(body)
                .post(endpoint)
    ...
    @Step("POST {endpoint}")
    public static Response postResponse(String endpoint, Map<String, Object> params) {
        return given()
                .auth().oauth2(tokenSupport)
                .queryParams(params)
                .post(endpoint)
